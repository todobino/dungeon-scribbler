...
export default function StorySoFarRefactoredPage() {
  ...
  const renderFormattedPlotPoints = (points: PlotPoint[]) => {
    if (points.length === 0) {
      return <p className="text-sm text-muted-foreground italic px-1 py-2">No plot points recorded for this session yet.</p>;
    }
    return (
      <ul className="space-y-3">
        {points.map(point => (
          <li key={point.id} className="p-3 border rounded-md bg-card shadow-sm">
      ...
      <AlertDialog open={isConfirmSessionAdvanceOpen} onOpenChange={setIsConfirmSessionAdvanceOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmAndAdvanceEmptySession} className={buttonVariants({variant: "default"})}>Yes, Start Session {currentSessionNumber + 1}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
