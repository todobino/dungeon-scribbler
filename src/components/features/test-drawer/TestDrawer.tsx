...
const SECONDARY_PANEL_CONTENT_WIDTH_VAL = 300; // Changed from 380
...
// Tailwind classes for the SheetContent's overall content width (excluding close bar padding)
const PRIMARY_SHEET_CONTENT_WIDTH_CLASS = `w-[${PRIMARY_PANEL_CONTENT_WIDTH_VAL}px]`;
const COMBINED_SHEET_CONTENT_WIDTH_CLASS = `w-[${PRIMARY_PANEL_CONTENT_WIDTH_VAL + SECONDARY_PANEL_CONTENT_WIDTH_VAL}px]`; // Recalculated: 380 + 300 = 680px
  ...
  return (
    <Sheet open={open} onOpenChange={handlePrimaryClose}>
      <SheetContent
        side="right"
        className={cn(
          "flex flex-col p-0 pr-8 overflow-hidden sm:max-w-none", // p-0 and pr-8 for main close bar
        ...
        <div className="flex flex-row h-full">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Main content of the Test Drawer. Its content area width is {PRIMARY_PANEL_CONTENT_WIDTH_VAL}px.
            </p>
            <Button 
              onClick={() => setIsSecondaryPanelVisible(prev => !prev)} 
            ...
            <div className="mt-auto">
              <p className="text-xs text-muted-foreground">
                ...
                <br />
                Primary Panel Width Class: {PRIMARY_PANEL_CLASS}
                <br />
                {isSecondaryPanelVisible && `Secondary Panel Width Class: ${SECONDARY_PANEL_CLASS}`}
              </p>
            </div>
          </div>
        </div>
